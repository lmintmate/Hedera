...
custom_distroicon() {
	...
	find "${_basedir}"/48/logos -name "distributor-*.png"|sed 's/^.*-//g;s/.png//g'
...
recolorfolders() {
	...
	read _newcolor
	if [ ! -f "${_basedir}"/48/folders/folder-$_newcolor.png ];then
		printf "\n$_newcolor is not a valid color! - Aborting!\n"
		exit 1
	else
		_oldcolor=$(readlink "${_basedir}"/48/misc-filesystems/inode-directory.png|sed 's/^.*-//g;s/\..*$//g')
...
#6: Exit this script\n\n"
	printf "Make your choice: [1,2,3,4,5,6,7,8,9]"
	read _choice
	case $_choice in
		1)
			clear
